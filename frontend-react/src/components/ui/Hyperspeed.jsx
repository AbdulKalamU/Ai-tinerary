...
const Hyperspeed = memo(({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      update(delta) {
        ...
        const scrollDelta = currentScrollY - (this.lastScrollY === undefined ? currentScrollY : this.lastScrollY);
        this.lastScrollY = currentScrollY;

        // Check if user is at the bottom of the page (end frame)
        const isAtBottom = (window.innerHeight + currentScrollY) >= document.documentElement.scrollHeight - 50;
        
        // Target speed is 0 when scrolling normally, but 1.5 when at the bottom CTA frame
        const speedTarget = isAtBottom ? 1.5 : 0;
        
        // Smoothly ramp the auto-play speed up or down
        this.speedUp = (this.speedUp || 0);
        this.speedUp += lerp(this.speedUp, speedTarget, lerpPercentage, 0.00001);
        
        // Accumulate virtual time based on both scroll delta and auto-play speed
        // This guarantees 0 lags, no snaps, and a seamless transition from scrub to auto-play!
        this.virtualTime = (this.virtualTime || 0) + (scrollDelta * 0.0025) + (this.speedUp * delta);

        let time = this.virtualTime;
