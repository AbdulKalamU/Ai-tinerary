...
public class GroqProvider implements AiProvider {
    ...
    @PostConstruct
    public void init() {
        log.info("=== Groq AI Provider Configuration ===");
    ...
    public String generateContent(String prompt) {
        if (!isConfigured()) {
            throw new RuntimeException("Groq provider is not properly configured. Check GROQ_API_KEY environment variable.");
        }
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                log.info("=== Groq API Request (Attempt {}/{}) ===", attempt, maxRetries);
    ...
    private String attemptGeneration(String prompt) throws Exception {
        ...
        // Build request using Jackson ObjectMapper for proper JSON serialization
        var requestNode = objectMapper.createObjectNode();
        requestNode.put("model", model);
        requestNode.put("temperature", 0.7);
        requestNode.put("max_tokens", 4500); // 4500 max_tokens + ~700 prompt = ~5200 < 6000 TPM limit
        ...
        log.info("Sending POST request to Groq API...");
        ...
        if (response.statusCode() != 200) {
            ...
            switch (response.statusCode()) {
                case 401:
                    throw new RuntimeException("Authentication failed. Check GROQ_API_KEY.");
                case 429:
        ...
        // Extract content using Jackson for proper JSON parsing
    ...
    /**
     * Extracts the AI-generated content from the Groq API response using Jackson ObjectMapper.
    ...
    private String extractContent(String responseBody) {
        try {
            ...
            log.error("Failed to extract content from Groq response", e);
